...
python run_redbluedoors_aif_ql.py --seed ${SEED} --episodes 1000 --max_steps 150 --change_every 50
EXIT_CODE=$?

if [ $EXIT_CODE -ne 0 ]; then
    echo "run_redbluedoors_aif_ql.py for seed $SEED failed with exit code $EXIT_CODE"
    exit $EXIT_CODE
fi

SEED_LOG_DIR="aif_ql_log_seed_${SEED}.csv"
