...
python run_redbluedoors_ql_random.py --seed ${SEED} --episodes 1000 --max_steps 150 --change_every 50
EXIT_CODE=$?

if [ $EXIT_CODE -ne 0 ]; then
    echo "run_redbluedoors_ql_random.py for seed $SEED failed with exit code $EXIT_CODE"
    exit $EXIT_CODE
fi

SEED_LOG_DIR="ql_rand_log_seed_${SEED}.csv"
