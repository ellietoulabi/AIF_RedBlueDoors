...
class Agent(object):
    ...
    def infer_states(self, observation, distr_obs = False):
        ...
        if self.inference_algo == "VANILLA":
            if self.action is not None: # if an action is taken
                empirical_prior = control.get_expected_states( # before seeing the observation, considering that the action is taken, where we expect the state to be
                    self.qs, self.B, self.action.reshape(1, -1) #type: ignore
                )[0]
            else: # if action is None (e.g.first time step)
                empirical_prior = self.D # use D (initial prior) 
            qs = inference.update_posterior_states( # given our prior for the next state, update the posterior based on the observation
            self.A, # observation likelihood
            observation, # observation
            empirical_prior, # prior for the next state (before seeing the observation)
