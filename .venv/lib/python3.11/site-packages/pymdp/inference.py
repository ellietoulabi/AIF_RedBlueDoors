...
def update_posterior_states(A, obs, prior=None, **kwargs):
    ...
    **kwargs: keyword arguments 
    ...
    num_obs, num_states, num_modalities, num_factors = utils.get_model_dimensions(A = A) # get the number of observation modalities, states, and factors
    
    obs = utils.process_observation(obs, num_modalities, num_obs) # process the observation and convert it to a one-hot vector if not already

    if prior is not None:
        prior = utils.to_obj_array(prior) # np.array(x, dtype=object) -> obj_array(x)

    return run_vanilla_fpi(A, obs, num_obs, num_states, prior, **kwargs) # run the fixed-point iteration algorithm
