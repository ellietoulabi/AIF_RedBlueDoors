...
def update_posterior_policies_full(
    ...
    for p_idx, policy in enumerate(policies):
        ...
        # print(f"Policy {p_idx} expected observations computed")

        if use_utility:
            G[p_idx] += calc_expected_utility(qo_seq_pi[p_idx], C)
        # print(f"Policy {p_idx} expected utility computed")
        if use_states_info_gain:
            G[p_idx] += calc_states_info_gain(A, qs_seq_pi[p_idx])
        # print(f"Policy {p_idx} expected states info gain computed")
...
def update_posterior_policies(
    ...
    for idx, policy in enumerate(policies):
        # print("get_expected_states Time start:", time.time())
        qs_pi = get_expected_states(qs, B, policy)
        # print("get_expected_states Time end:", time.time())
        # print("get_expected_obs Time start:", time.time())
        qo_pi = get_expected_obs(qs_pi, A)
        # print("get_expected_obs Time end:", time.time())

        if use_utility:
            # print("calc_expected_utility Time start:", time.time())
            G[idx] += calc_expected_utility(qo_pi, C)
            # print("calc_expected_utility Time end:", time.time())

        if use_states_info_gain:
            # print("calc_states_info_gain Time start:", time.time())
            G[idx] += calc_states_info_gain(A, qs_pi)
            # print("calc_states_info_gain Time end:", time.time())
