...
def run_vanilla_fpi(A, obs, num_obs, num_states, prior=None, num_iter=10, dF=1.0, dF_tol=0.001):
    ...
    # This is computing the likelihood of a state given an observation across multiple modalities.
    # P(observation₁, observation₂, ..., observationₙ | state) 
    # conditionally independent assumption
    # P(observation₁, observation₂, ..., observationₙ | state) = P(observation₁ | state) × P(observation₂ | state) × ... × P(observationₙ | state)
    # The multiplication combines the likelihoods from all these modalities to get the overall likelihood of the current state given all your observations.
    ...
    # NOTE: el commented this for a faster version
    # qs = np.empty(n_factors, dtype=object)
    # for factor in range(n_factors):
    #     qs[factor] = np.ones(num_states[factor]) / num_states[factor]
    qs = np.array([np.ones(n) / n for n in num_states], dtype=object)
