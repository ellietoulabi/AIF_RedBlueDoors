...
def dot_likelihood(A,obs):
    ...
    return LL
def el_dot_likelihood(A, obs):
    # Handles both one-hot and categorical index
    if obs.ndim == 0 or (obs.ndim == 1 and obs.size == 1):
        obs_idx = int(obs)
    else:
        obs_idx = np.argmax(obs)
    return np.asarray(A[obs_idx])
...
def get_joint_likelihood(A, obs, num_states):
    ...
    for modality in range(len(A)):
        ...
        ll = ll * el_dot_likelihood(A[modality], obs[modality]) # gives the likelihood of stated for a specific observation of this modality A[m][obs_idx]
